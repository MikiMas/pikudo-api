...
import { sendTelegramMessage } from "@/app/api/(telegram)/telegram/_lib/bot";
...
export async function GET(req: Request) {
  ...
  if (token) {
    ...
    if (order?.chat_id) {
      await sendTelegramMessage(order.chat_id, "Pago cancelado. Si quieres intentarlo de nuevo, escribe /start.");
      await supabase
        .from("paypal_orders")
