...
export async function POST(req: Request) {
  try {
    ...
    const { error: playerError } = await supabase.from("players").update({ room_id: null }).eq("id", playerId);
    if (playerError) return apiJson(req, { ok: false, error: "LEAVE_FAILED" }, { status: 500 });
