...
export async function POST(req: Request) {
  try {
    ...
    if (pcError) return apiJson(req, { ok: false, error: "UPLOAD_FAILED" }, { status: 500 });
    ...
    if (uploadError) {
      return apiJson(req, { ok: false, error: "UPLOAD_FAILED", hint: `Crea el bucket '${BUCKET}' en Supabase Storage.` }, { status: 500 });
    }
    ...
    if (updateError) return apiJson(req, { ok: false, error: "UPLOAD_FAILED" }, { status: 500 });
